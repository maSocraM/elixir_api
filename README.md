...
### 6.4. Conclusões
Se mostrou uma linguagem e framework muito produttivos, porém, quando se é necessário produzir além do que é automático, é uma tarefa árdua e, pelo que busquei de informações, sem muitas fontes.
Para uso inicial eu recomendaria a criação de pequenos serviços simples e buscaria capacitação para os envolvidos, assim como já acontece na maioria das linguagens de programação existentes, é necessário um longo tempo de contato e estudos para poder conhecer mais e assim conseguir tirar o melhor e conhecer mais os limites, tanto da linguagem quanto do framework, uma semana de dedicação parcial não foi o suficiente no meu caso.
Eu acredito que eu teria melhores resultados, no que se refere a produtividade e alcance dos desafios propostos, utilizando a linguagem de programação Rusty, pelo fato de eu já ter algum contato no passado e também por ela estar dentro dos paradigmas que são mais de meu domínio.
